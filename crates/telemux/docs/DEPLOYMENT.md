...
- `target/release/telemux-sim` / `telemux-sim.exe` —— **CDU 模拟器**（独立可执行：
  物理模型 + Modbus-TCP 从站 + 网页 UI，见 `docs/SIMULATION.md`）；
- `target/release/examples/mock_pcba` —— 仅开发桩（release 下打印提示后退出）。

Release 优化（见根 `Cargo.toml [profile.release]`）：
- `lto = true` + `codegen-units = 1`：全程序链接优化，体积更小；
- `strip = true`：剥离符号（Windows 下 telemux.exe 约 2.7 MB，telemux-sim.exe 约 1.8 MB）；
- `panic = "abort"`：panic 直接中止，无 unwind 开销（嵌入式/网关场景更稳）。

**Release 网关二进制不含任何开发/模拟功能**（已字符串级验证）：dashboard 模块、
mock PCBA、SimSource/SimConfig/telemux-sim 全部无痕（workspace 彻底解耦）。
