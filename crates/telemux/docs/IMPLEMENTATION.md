...
**文档**：`docs/SIMULATION.md`、模拟器配置 `crates/telemux-sim/config/cdu.toml`、
网关对接配置 `config/cdu-gateway.toml`。

把"假 mock"升级为**独立运行的 CDU 模拟器**，并用 **cargo workspace** 拆分
（单一 Cargo.lock 保证两 crate 依赖版本一致）：

- **workspace**：`crates/telemux`（网关）+ `crates/telemux-sim`（模拟器）；
  `git mv` 保历史迁入；网关 release 二进制**字符串级验证无任何 sim 痕迹**（彻底解耦）
- **模拟器（telemux-sim）**：`model.rs`（SimConfig + 稳态求值，从网关迁出）、
  `registers.rs`（寄存器地图：保持区=控制变量 u16 可写，输入区=传感器 f32 双字）、
  `server.rs`（Modbus-TCP 从站）、`web.rs`（网页 UI：寄存器地址/类型/原始值表，
  1s 刷新，`GET /api/state` JSON）
- **网关彻底解耦**：删除 `Transport::Sim` 进程内源与 `[sim]` 配置段；网关只保留
  tcp/rtu，用 `config/cdu-gateway.toml` 以 `transport="tcp"` 连接模拟器，与真实 CDU 同构
- **物理建模原则**：物理点（Pn/Tn/Fn 独立测量位置）在模拟器 `[sim]`，
  **压差/温差等派生量用网关 `[[computed]]`**（真实系统无 dp 探头，是两路压力差分）——
  生产切换时 computed 原样保留
- **变量解析**：`inputs` 映射（与 computed 一致）→ 控制变量名 → 内置时间 `t` → 传感器短名
- 模拟器配置：29 传感器（一次/二次侧 P/T/F、泵进出口压力、水箱液位/PH、环境/泄漏）
  + 4 控制变量（pump1/2_duty、valve1_duty、fan_duty）

**验收（已端到端验证）**：模拟器（1502 + 网页 8082）与网关（cdu-gateway.toml）
双进程运行：Redfish Sensors 17（14 寄存器 + 3 computed）、readyz `sensors_total:14`；
**完整闭环**：网关 Modbus 写 pump1_duty=90 → 模拟器模型 → f2=153 L/min；
`tests/cdu_sim.rs` 3 个集成测试（模拟器读写 + 网关→模拟器闭环）通过；
86 测试全绿、clippy 干净、workspace release 构建通过（telemux 2.7MB / telemux-sim 1.8MB）。
