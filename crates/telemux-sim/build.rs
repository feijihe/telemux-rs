...
//!
//! 若 `web/apps/sim-ui` 尚未执行 `pnpm run build`，则生成一个最小占位
...
<html lang="zh-CN">
<head><meta charset="UTF-8"><title>Telemux-Sim</title></head>
<body style="font-family: monospace; padding: 32px;">
  <h1>Telemux-Sim · CDU 仿真器</h1>
  <p>前端尚未构建：请先运行 <code>pnpm --filter sim-ui build</code>（在仓库根 <code>web/</code> 目录），
  然后重新 <code>cargo build</code>。</p>
</body>
</html>
"#;
        fs::write(dist.join("index.html"), placeholder).expect("write placeholder index.html");
        println!("cargo:warning=web/dist 不存在，已生成占位 index.html（请运行 pnpm --filter sim-ui build 构建前端）");
    }
}
