...
- `web/` — **pnpm workspace** (React + TS + shadcn/ui): `apps/sim-ui` (simulator UI), `apps/dashboard` (gateway dev dashboard), `packages/ui` (shared components/types). Build output lands in each crate's `web/dist`, embedded at compile time via `include_dir!` (`crates/*/src/**/web_assets.rs`); `build.rs` generates a placeholder if dist is missing. Shared components are managed by the shadcn CLI: `components.json` lives in `packages/ui/` (style `radix-nova`), source in `packages/ui/src/components/ui/`; add/update with `npx shadcn@latest add <component> -c packages/ui`. Note: `pnpm dlx shadcn@latest` currently crashes on this machine (MCP SDK zod resolution) — use `npx shadcn@latest` instead. App code imports shadcn components via `@telemux/ui`; the `@/` alias in both apps' `vite.config.ts`/`tsconfig.json` points at `packages/ui/src` because the generated component files import each other via `@/lib/utils` and `@/components/ui/*`.
